    ...
    foreach($item->children() as $category){ # Go through each "child" tag of a given item in the orginal feed and take the follwing steps
        if($category->getName() == 'category'){  # IF the tag is a category... 
            ...
            if($categoryHashtags[$categoryString]){ # Look in the category replacement file for a replacement hashtag, and IF it exists do the folowing 
                ...
                if(strlen($r.$h.$categoryHashtags[$categoryString])<=80 and strlen($categoryHashtags[$categoryString])>=3) # If the current string + the new hashag are less then (38) 80 characters long Added by Neil-> If the replacement category hastag is as long as 3 characters
                ...
                if(strlen($r.$t)<=80) #As long as the current string + t above is under (38) 80 characters 
...
$feedXML = preg_replace('/&lt;title&gt;(.{1,170})(.+)&lt;\/title&gt;/u', "&lt;title&gt;$1&lt;/title&gt;", $feedXML); # Replace the titles after the first (70) 170 with a number. 
